...
$block = {   
    ...
    $configFilename = 'watcher_config.json'
    ...
    function Copy-Image
    {
        ...
        try {
            ...
            # $unixTimeStamp = [int][double]::Parse((Get-Date -Date $timeStamp -UFormat %s))
            $uuid = [guid]::NewGuid()
            $shortUuid = $uuid.ToString().Substring(0, 6)
            $id = "${formattedDate}_${shortUuid}"
...
if ( $encodedBlock.Length -gt 32768 ) { Write-Host "Encoded command is too long: ${encodedBlock.Length} > 32768" }

$scriptLocation = Split-Path -Parent $MyInvocation.MyCommand.Definition
Start-Process PowerShell.exe -WorkingDirectory $scriptLocation -verb Runas -argumentlist '-NoExit', "-ExecutionPolicy Bypass", '-EncodedCommand', $encodedBlock
#  '-WindowStyle Hidden'
